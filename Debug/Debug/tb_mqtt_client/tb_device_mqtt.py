...
from tb_mqtt_client.sdk_utils import verify_checksum
...
class TBDeviceMqttClient:
    ...
    def _on_connect(self, client, userdata, flags, result_code, *extra_params):
        if result_code == 0:
            try:
                client_id_with_sep = self.gateway.config.get("security").get("clientId") + "/"
            except Exception as e:
                client_id_with_sep = ''
                log.error("Error while read clientId from config file", e)
            ...
            self._subscribe_to_topic(RPC_REQUEST_TOPIC + client_id_with_sep + '+',
                                     qos=self.quality_of_service)  # 追加客户id到topic, updated by jackiekuang
    ...
    def _on_decoded_message(self, content, message):
        if message.topic.startswith(RPC_REQUEST_TOPIC):
            ...
            request_id = message.topic.split('/')[
                -1]  # message.topic[len(RPC_REQUEST_TOPIC):len(message.topic)] updated by jackie kuang
        ...
        if message.topic.startswith("v1/devices/me/attributes"):
            ...
            if ((self.firmware_info.get(FW_VERSION_ATTR) is not None
                 and self.firmware_info.get(FW_VERSION_ATTR) != self.current_firmware_info.get(
                        "current_" + FW_VERSION_ATTR))
                    or (self.firmware_info.get(FW_TITLE_ATTR) is not None
                        and self.firmware_info.get(FW_TITLE_ATTR) != self.current_firmware_info.get(
                                "current_" + FW_TITLE_ATTR))):
    ...
    def _wait_until_current_queued_messages_processed(self):
        ...
        inflight_messages = self._client._max_inflight_messages or 30 #5=>30 updated by jackie kuang
        logger = None
    ...
    def __send_split_message(self, results, part, kwargs, timeout, device, msg_rate_limit, dp_rate_limit,
        ...
        # payload消息格式转换，和EMQX解析保持一致，updated by jackie kuang
        # e.g.: {"sun21971918034_A_AHU_09F_1":[{"ts":1736323213290,"values":{"RunningStatus":1,"RunningFrequency":29.65}}]}
        if device is not None and topic == 'v1/gateway/telemetry':
            try:
                message = part['message'][device][0]
                ts = message['ts']
                values = []
                for (p, v) in message['values'].items():
                    values.append({'param_name': p, 'value': v})
                payload = {'device': device, 'ts': ts, 'data': values}
                kwargs["payload"] = dumps(payload)
            except Exception as e:
                kwargs["payload"] = dumps(part['message'])  # 使用默认值
        else:
            kwargs["payload"] = dumps(part['message'])
    ...
    def __timeout_check(self):
        while not self.stopped:
            ...
            for (attr_request_number, ts) in tuple(self.__attrs_request_timeout.items()):
                ...
                if callback is not None:
                    if isinstance(callback, tuple):
                        callback[0](None, TBTimeoutException(
                            "Timeout while waiting for a reply for attribute request from ThingsBoard!"),
                                    callback[1])
                    else:
                        callback(None, TBTimeoutException(
                            "Timeout while waiting for a reply for attribute request from ThingsBoard!"))
    ...
    def _split_message(message_pack, datapoints_max_count, max_payload_size):
        ...
        for (message_index, message) in enumerate(message_pack):
            ...
            for current_data_key_index, data_key in enumerate(values_data_keys):
                ...
                if ((TBDeviceMqttClient._datapoints_limit_reached(datapoints_max_count,
                                                                  len(message_item_values_with_allowed_size),
                                                                  current_size))
