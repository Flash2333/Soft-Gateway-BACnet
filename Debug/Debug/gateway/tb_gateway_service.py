...
from os import execv, listdir, path, pathsep, stat, system, getcwd, environ
...
from gateway.entities.datapoint_key import DatapointKey
from gateway.entities.report_strategy_config import ReportStrategyConfig
from gateway.report_strategy.report_strategy_service import ReportStrategyService
from gateway.shell.proxy import AutoProxy
from gateway.statistics.decorators import CountMessage, CollectStorageEventsStatistics, \
...
from tb_utility.tb_handler import TBRemoteLoggerHandler
...
class TBGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        if config_file is None:
            config_file = get_config_path() + 'gateway.json'
            # config_file = (path.dirname(path.dirname(path.abspath(__file__))) + '/config/gateway.json'.replace('/', path.sep))
        ...
        self._event_storage = self._event_storage_types[self.__config["storage"]["type"]](self.__config["storage"],
                                                                                          storage_log)
        self._report_strategy_service = ReportStrategyService(self.__config['cloudServer'],
                                                              self,
        ...
        self.__latency_debug_mode = self.__config['cloudServer'].get('latencyDebugMode', False)
        ...
        self.tb_client = TBClient(self.__config["cloudServer"], self._config_dir, connection_logger)
        ...
        self.init_remote_shell(self.__config["cloudServer"].get("remoteShell"))
        ...
        self.__devices_idle_checker = self.__config['cloudServer'].get('checkingDeviceActivity', {})
        ...
        self.init_statistics_service(self.__config['cloudServer'].get('statistics', DEFAULT_STATISTIC))

        self.__min_pack_send_delay_ms = self.__config['cloudServer'].get('minPackSendDelayMS', 50)
        self.__min_pack_send_delay_ms = self.__min_pack_send_delay_ms / 1000.0
        self.__min_pack_size_to_send = self.__config['cloudServer'].get('minPackSizeToSend', 500)
        self.__max_payload_size_in_bytes = self.__config["cloudServer"].get("maxPayloadSizeBytes", 8196)

        self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                                   name="Send data to cloudServer Thread")
        self._send_thread.start()

        self.init_device_filtering(self.__config['cloudServer'].get('deviceFiltering', DEFAULT_DEVICE_FILTER))
        ...
        if self.__config['cloudServer'].get('managerEnabled', False):
        ...
        try:
            ...
            while not self.stopped:
                try:
                    ...
                    if (cur_time - connectors_configuration_check_time > self.__config["cloudServer"].get("checkConnectorsConfigurationInSeconds", 60) * 1000 # noqa
    ...
    def __init_remote_configuration(self, force=False):
        remote_configuration_enabled = self.__config["cloudServer"].get("remoteConfiguration")
                    ...
                    if events:
                        ...
                        if devices_data_in_event_pack:
                            ...
                            if self.__latency_debug_mode and events_len > 100:
                                ...
                                log.debug("Sending data to cloudServer, pack size %i processing took %i ,milliseconds. Average event processing time is %s",  # noqa
                                          events_len,
    ...
    def __handle_published_events(self):
        ...
        try:
            ...
            for success in futures:
                event_num += 1
                if event_num % 100 == 0:
                    log.debug("Confirming %i event sent to cloudServer", event_num)
                if not success:
                    return False

            return True
        except Exception:  # noqa
            log.debug("Error while sending data to cloudServer, it will be resent.")
            return False
    ...
    def __process_published_event(event):
        try:
            ...
            log.error("Error while sending data to cloudServer, it will be resent.", exc_info=e)
            return False
        except Exception as e:
            log.error("Error while sending data to cloudServer, it will be resent.", exc_info=e)
            return False
        ...
        while not self.stopped:
            try:
                ...
                if monotonic() - received_time > timeout:
                    ...
                    #self.send_rpc_reply(content["device"], request_id, "{\"error\":\"Request timeout\", \"code\": 408}") commented by jackie kuang
                device = content.get("device")
                if device in self.get_devices():
                    ...
                    log.error("Failed to call RPC request as device '%s' not existing", device)
                    # 修复当设备号不存在时死循环的bug
...
if __name__ == '__main__':
    TBGatewayService(get_config_path() + 'gateway.json')
        # path.dirname(path.dirname(path.abspath(__file__))) + '/config/gateway.json'.replace('/', path.sep))
