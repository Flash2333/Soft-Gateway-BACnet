...
class Application(App):
    ...
    async def do_who_is(self, device_address):
        addr = Address(device_address) if device_address is not None else None
        discovery_timeout = self.__device_object_config.device_discovery_timeout
        if device_address is None: #广播的方式，超时时间延长一倍
            discovery_timeout *= 2
        devices = await self.who_is(address=addr, timeout=discovery_timeout)

        if len(devices):
            return devices[0] if device_address is not None  else devices
